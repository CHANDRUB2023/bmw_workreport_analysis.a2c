...
export default function ProductivityAnalysisCard() {
  ...
  return (
    <section className="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs space-y-4 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-200/90 pb-3 gap-3">
        <div>
          <div className="flex items-center gap-2">
            <span className="px-2 py-0.5 text-xs font-black bg-blue-100 text-blue-800 rounded-md">SEC 07</span>
        ...
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-50/90 border border-amber-300 rounded-xl text-xs font-bold text-amber-900 shadow-2xs">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center bg-slate-50/80 border border-slate-200/90 rounded-2xl p-4 shadow-2xs">
        <div className="md:col-span-6 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-black text-blue-900 px-2.5 py-0.5 bg-white border border-blue-200 rounded-lg shadow-2xs">
              {selectedHours} Hours
            </span>
          </div>
          <input
        ...
        <div className="md:col-span-6 grid grid-cols-3 gap-2 text-center">
          <div className="bg-white border border-slate-200/80 rounded-xl p-2.5 shadow-2xs">
            ...
            <span className="text-lg font-black text-slate-800">
              <AnimatedCounter value={activeData.minOutput} />
            </span>
          ...
          <div className="bg-blue-50/90 border border-blue-200 rounded-xl p-2.5 shadow-2xs">
            ...
            <span className="text-lg font-black text-blue-900">
              <AnimatedCounter value={activeData.avgOutput} />
            </span>
            <span className="text-[10px] text-blue-700 block">Venues</span>
          </div>

          <div className="bg-emerald-50/90 border border-emerald-200 rounded-xl p-2.5 shadow-2xs">
            <span className="text-[10px] font-bold text-emerald-700 uppercase block">Max (2.5/h)</span>
            <span className="text-lg font-black text-emerald-900">
              <AnimatedCounter value={activeData.maxOutput} />
            </span>
      ...
      <div className="h-72 w-full pt-2">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={series} margin={{ top: 10, right: 30, left: 10, bottom: 5 }}>
            ...
            <Tooltip
              content={({ active, payload, label }) => {
                if (active && payload && payload.length) {
                  return (
                    <div className="bg-slate-900/95 backdrop-blur-md text-white p-3 rounded-xl text-xs shadow-2xl border border-slate-700 space-y-1">
            ...
            <Line type="monotone" dataKey="minOutput" name="Minimum (1.0/h)" stroke="#f59e0b" strokeWidth={2} dot={{ r: 3 }} isAnimationActive={true} animationDuration={1200} />
            <Line type="monotone" dataKey="avgOutput" name="Average (1.75/h)" stroke="#2563eb" strokeWidth={3} dot={{ r: 4 }} isAnimationActive={true} animationDuration={1200} />
            <Line type="monotone" dataKey="maxOutput" name="Maximum (2.5/h)" stroke="#10b981" strokeWidth={2} dot={{ r: 3 }} isAnimationActive={true} animationDuration={1200} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </section>
