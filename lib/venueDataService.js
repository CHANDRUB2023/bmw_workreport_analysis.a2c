...
function parseExcelSheets(wb) {
  ...
  return wb.SheetNames.map(sheetName => {
    ...
    for (let c = 0; c < maxCols; c++) {
      let header = (r0[c] !== undefined && r0[c] !== null) ? String(r0[c]).trim() : '';

      if (!header && r1[c] !== undefined && r1[c] !== null && typeof r1[c] === 'string') {
        const valStr = String(r1[c]).trim();
        if (isNaN(Number(valStr))) {
          header = valStr;
        }
      }

      const startRow = (!r0[c] && header === String(r1[c]).trim()) ? 2 : 1;
      const valList = [];
      ...
      if (!header && valList.length === 0) continue;
      if (!header) header = `Column ${c + 1}`;

      const uniqueValuesMap = new Map();
      valList.forEach(v => {
        const strKey = String(v).trim();
        if (!uniqueValuesMap.has(strKey)) {
          uniqueValuesMap.set(strKey, v);
        }
      });

      const type = classifyFieldType(valList);
      const actualValues = Array.from(uniqueValuesMap.values());
      ...
      fields.push({
        ...
        uniqueCount: uniqueValuesMap.size,
        sampleValues: actualValues.slice(0, 15),
...
export function getVenueAnalyticsSummary() {
  ...
  const excelSheets = parseExcelSheets(wb);
  ...
  // Dynamic Columns from parsed sheets
  const dynamicColumns = excelSheets.length > 0 ? excelSheets[0].fields : [];
