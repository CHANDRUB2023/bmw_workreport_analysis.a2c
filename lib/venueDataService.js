...
export function getVenueAnalyticsSummary() {
  ...
  // 2. Scan Dynamic Additional Columns in Header Row
  const headerRow = rows[0] || [];
  const knownHeaders = ['DISTRICTS', 'DISTRICT', 'VENUE COUNT', 'VENUES', 'VENUE', 'STATE', 'UNION TERRITORIES', 'UNION TERRITORIES (DISTRICTS)', 'STATUS LABELS'];
  const dynamicColMap = {};
  ...
  headerRow.forEach((cell, c) => {
    if (cell && typeof cell === 'string' && c >= distCol) {
      ...
      if (!knownHeaders.includes(upper)) {
