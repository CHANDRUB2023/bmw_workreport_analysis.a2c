...
function resolveExcelPath() {
  ...
  const defaultDownloadsPath = 'C:\\Users\\chand\\Downloads\\TN Districts Pincodes.xlsx';
  ...
  candidates.push(defaultDownloadsPath);
  candidates.push(path.resolve(defaultDownloadsPath));
  candidates.push('C:/Users/chand/Downloads/TN Districts Pincodes.xlsx');
...
export function getVenueAnalyticsSummary() {
  ...
  for (let r = 0; r < Math.min(5, rows.length); r++) {
    ...
    row.forEach((cell, c) => {
      if (typeof cell === 'string') {
        ...
        if (val === 'DISTRICTS' && distCol === -1) distCol = c;
        if (val === 'VENUE COUNT' && venueCol === -1 && c !== distCol) venueCol = c;
      }
    });
  ...
  // 2. Extract District Unique Pincodes from columns prior to distCol
  ...
  // 3. Extract Tamil Nadu District Venue Records
  const rawDistricts = [];
  for (let r = 1; r < rows.length; r++) {
    const row = rows[r];
    if (!row) continue;
  ...
  // 4. Union Territory Data Extraction
  let utCol = -1;
  let utVenueCol = -1;
